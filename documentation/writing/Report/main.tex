...
In the case of this project's dataset, the process is clearly non-stationary as the firing rate varies depending on the time windows. 
...
Consequently, from now on it is assumed that the rate at which a neuron fires depends on the intensity of firing that it had just before.        
...
Given the autocorrelation function \textit{x(n)}, the power spectral density \begin{math}S(m) = |X(m)|^2\end{math}, where \textit{X(m)} is the discrete Fourier transform of \textit{x(n)}. 
...
The non-stationarity of the spiking rate over time makes it impossible to model the spike count trains as a homogeneous Poisson process with a constant rate; therefore, it will be regarded as an inhomogeneous Poisson process.
...
We can also see an estimation of the rate probability distribution by plotting a histogram of the time series.
...
Next, the power spectral density was drawn from the autocorrelation measure, by calculating the square of its Fourier transform. 
Lastly, using the power spectral density values, the spectral entropy of each channel were obtained.
